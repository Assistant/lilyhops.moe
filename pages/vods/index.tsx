...
export default function Vods(props: VodsProps) {
  ...
  const getMoreVods = async () => {
    const res = await fetch(`https://${window.location.hostname}/api/vods?start=${vods.length}`)
...
export const getServerSideProps: GetServerSideProps  = async (context: GetServerSidePropsContext) => {
  const vodsData: VodData[] = peal('vod')(getVods({root: './public/lily', regex: /v(\d+).mp4$/, type: 'vod'}))
  return {
    props: {
